...
export function mountKnowledgeAPI(app:Hono,services:{knowledge:KnowledgeBase;indexer:Indexer;analyses:AnalysisManager;providers:Providers;config:Config;json:(c:any)=>Promise<any>}) {
  ...
  const source=(id:string)=>{const s=kb.source(id);if(!s)throw new AppError('NOT_FOUND','Source connection not found.',404);return s;};
  ...
  app.post('/api/v1/projects/:id/versions',async c=>{const body=await json(c);if(body?.sourceIds!==undefined&&(!Array.isArray(body.sourceIds)||body.sourceIds.some((x:any)=>typeof x!=='string')))throw new AppError('INVALID_SOURCES','Provide a valid list of source IDs.');if(body?.snapshots!==undefined&&(!body.snapshots||typeof body.snapshots!=='object'||Array.isArray(body.snapshots)||Object.values(body.snapshots).some(x=>typeof x!=='string')))throw new AppError('INVALID_SNAPSHOT','Select valid snapshots.');return c.json({job:indexer.enqueueVersion({...body,projectId:c.req.param('id')})},202);});
  app.get('/api/v1/versions/:id',c=>{const v=kb.version(c.req.param('id'));if(!v)throw new AppError('NOT_FOUND','Software version not found.',404);return c.json({version:v,documents:Object.values(v.snapshots).flatMap(id=>kb.snapshotFiles(id).map(f=>({id:f.id,path:f.path,sourceId:f.sourceId,moduleId:f.moduleId,role:f.role,snapshotId:id})))});});
  app.delete('/api/v1/versions/:id',c=>{const id=c.req.param('id');if(indexer.versionBusy(id)||analyses.list().some(j=>['queued','running'].includes(j.status)&&j.versionIds.includes(id)))throw new AppError('VERSION_BUSY','Wait for indexing, snapshot, and analysis jobs using this version to finish before deleting it.',409);kb.deleteVersion(id);return c.json({deleted:true});});
  ...
  app.post('/api/v1/sources/git',async c=>{
    ...
    if(kb.sources().some(s=>s.projectId===projectId&&s.git?.url===git.url&&s.git?.branch===git.branch))throw new AppError('DUPLICATE_SOURCE','This repository and branch are already connected.',409);
    const id=crypto.randomUUID(),fallbackName=git.url.split('/').at(-1)!.replace(/\.git$/,'');
    const rawName=body?.name??'';
    if(typeof rawName!=='string'||rawName.length>120||/[\x00-\x1f]/.test(rawName))throw new AppError('INVALID_FIELD','Enter a valid source name.');
  ...
  app.post('/api/v1/sources/:id/settings',async c=>{
    const s=source(c.req.param('id')),body=await json(c);if(body?.paused!==undefined&&typeof body.paused!=='boolean')throw new AppError('INVALID_FIELD','The paused value must be true or false.');
    if(body?.documentLinks!==undefined){if(!body.documentLinks||Array.isArray(body.documentLinks)||typeof body.documentLinks!=='object'||Object.entries(body.documentLinks).some(([p,v])=>typeof v!=='string'||p.length>1024||v.length>1024||/[\x00-\x1f]/.test(p+v)))throw new AppError('INVALID_LINKS','Provide valid mappings between current and previous document paths.');}
  ...
  app.get('/api/v1/analyses/:id',c=>{const job=analyses.get(c.req.param('id'));if(!job)throw new AppError('NOT_FOUND','Analysis job not found.',404);return c.json({job});});
