...
같은 데이터 폴더의 세션이 다른 포트·모델 모드·바인딩 주소로 실행 중이면 설정 불일치를 안내하고 기존 세션을 유지합니다. 설정을 바꾸려면 해당 `-DataDirectory`로 먼저 중지한 뒤 다시 시작하세요. 예전 실행 기록에는 바인딩 주소가 없어 loopback과 전체 인터페이스 바인딩의 차이를 구별하지 못할 수 있으므로, 네트워크 설정 변경 시 반드시 중지 후 시작합니다.
